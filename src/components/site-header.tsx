...
export function SiteHeader() {
  ...
  const closeMenus = () => {
    if (digitalMenuRef.current) digitalMenuRef.current.open = false;
    ...
    <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/95 backdrop-blur">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        ...
        <nav className="hidden items-center gap-5 text-sm font-semibold text-slate-700 2xl:flex">
          ...
          <details ref={digitalMenuRef} className="group relative">
            ...
            <div className="absolute left-0 top-8 w-56 rounded-2xl border border-slate-200 bg-white p-2 shadow-soft">
              {digitalItems.map((item) => <Link key={item.label} href={item.href} onClick={closeMenus} className="block rounded-xl px-4 py-3 font-bold text-dgNavy hover:bg-dgIvory">{item.label}</Link>)}
            </div>
          </details>
          {menuItems.slice(2).map((item) => <Link key={item.label} href={item.href} onClick={closeMenus} className="transition hover:text-dgGreen">{item.label}</Link>)}
        </nav>
        <div className="flex items-center gap-2">
          ...
          <details ref={mobileMenuRef} className="group relative 2xl:hidden">
            ...
            <div className="absolute right-0 top-14 max-h-[75vh] w-[min(24rem,calc(100vw-2rem))] overflow-y-auto rounded-2xl border border-slate-200 bg-white p-3 shadow-soft">
              {menuItems.slice(0, 2).map((item) => <Link key={item.label} href={item.href} onClick={closeMenus} className="block rounded-xl px-4 py-3 font-bold text-dgNavy hover:bg-dgIvory">{item.label}</Link>)}
              <div className="my-2 rounded-xl bg-dgIvory p-2">
                ...
                {digitalItems.map((item) => <Link key={item.label} href={item.href} onClick={closeMenus} className="block rounded-lg px-3 py-2.5 font-bold text-dgNavy hover:bg-white">{item.label}</Link>)}
              </div>
              {menuItems.slice(2).map((item) => <Link key={item.label} href={item.href} onClick={closeMenus} className="block rounded-xl px-4 py-3 font-bold text-dgNavy hover:bg-dgIvory">{item.label}</Link>)}
            </div>
          </details>
        </div>
      </div>

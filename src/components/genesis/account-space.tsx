...
function ZumraPanel({ data, displayName, userId }: { data: ZumraMePayload | null; displayName: string; userId: string }) {
  if (data?.enrolled && data.membership?.status === 'active') {
    return <div className="overflow-hidden rounded-[2rem] border border-dgGold/30 bg-dgNavy p-7 text-white sm:p-8"><div className="flex items-start justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-dgGold">Ma Carte ZUMRA</p><h2 className="mt-3 text-2xl font-black">{displayName}</h2></div><CreditCard className="h-8 w-8 text-dgGold" /></div><div className="mt-7 rounded-2xl border border-white/10 bg-white/5 p-5"><p className="text-xs font-bold uppercase tracking-[0.15em] text-slate-400">ID membre</p><p className="mt-2 break-all font-black">{userId}</p><div className="mt-5 grid gap-4 sm:grid-cols-2"><CardInfo label="Pays" value={data.profile?.country || '—'} /><CardInfo label="Statut" value="Membre actif" /><CardInfo label="Reseau" value="Disponible" /><CardInfo label="Contribution" value={contributionLabel(data.membership.contributionStatus)} /></div></div><div className="mt-5 flex flex-wrap gap-3"><Link href="/espace/zumra/reseau" className="rounded-xl bg-dgGold px-4 py-3 text-sm font-black text-dgNavy">Ouvrir le reseau ZUMRA</Link><Link href="/espace/zumra" className="rounded-xl border border-white/15 px-4 py-3 text-sm font-bold text-white">Mon profil ZUMRA</Link><span className="rounded-xl border border-white/15 px-4 py-3 text-sm font-bold text-slate-300">Carte physique : prochaine etape</span></div></div>;
  }

  if (data?.enrolled && data.membership?.status === 'pending_payment') {
    return <div className="rounded-[2rem] border border-amber-200 bg-amber-50 p-7 sm:p-8"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-white text-amber-700"><Network className="h-6 w-6" /></div><p className="mt-5 text-xs font-black uppercase tracking-[0.18em] text-amber-700">Programme ZUMRA</p><h2 className="mt-3 text-2xl font-black text-dgNavy">Adhesion a finaliser</h2><p className="mt-3 leading-7 text-slate-600">Votre profil est enregistre. Finalisez l’adhesion dans le sandbox GeniusPay ; votre Carte ZUMRA ne sera activee qu’apres confirmation serveur du paiement.</p><Link href="/espace/zumra" className="mt-5 inline-flex rounded-xl bg-dgNavy px-4 py-3 text-sm font-black text-white">Finaliser mon adhesion</Link></div>;
  }

...
import { ArrowRight, BriefcaseBusiness, Globe2, Handshake, Send } from 'lucide-react';
    ...
    <section id="deposer" className="bg-white px-4 py-20 sm:px-6 lg:px-8">
      <div className="mx-auto grid max-w-7xl gap-10 lg:grid-cols-[.75fr_1.25fr]">
        <div>
          ...
          <h2 className="mt-3 text-3xl font-black text-dgNavy sm:text-4xl">Vous avez une offre, un besoin ou un projet à connecter ?</h2>
          <p className="mt-5 leading-8 text-slate-600">Déposez les informations essentielles. Votre proposition arrive en brouillon dans l’administration DG AFRIQUE et n’est jamais publiée automatiquement. Notre équipe la vérifie avant toute mise en ligne.</p>
        ...
        <form action={submitOpportunityAction} className="grid gap-4 rounded-[2rem] border border-slate-200 bg-dgIvory p-5 sm:grid-cols-2 sm:p-8">
          ...
          <div><label className="mb-2 block text-xs font-black uppercase tracking-[0.1em] text-slate-500">Secteur</label><input name="sector" required className={inputClass} placeholder="Agriculture, BTP, Mines..." /></div>

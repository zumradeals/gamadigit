  ...
  return (
    <article className="flex h-full flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-soft">
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <h3 className="mt-4 text-2xl font-black text-ink">{product.name}</h3>
        ...
        <div className="mt-auto border-t border-slate-100 pt-6">
          ...
          >
            ...
            Commander ou demander le tarif
          </a>
        </div>
      </div>
    </article>
      ...
      <section className="relative overflow-hidden bg-ink px-4 py-20 text-white sm:px-6 lg:px-8 lg:py-24">
        ...
        <div className="relative mx-auto max-w-7xl">
          ...
          <h1 className="mt-4 max-w-4xl text-4xl font-black tracking-tight sm:text-6xl">
            Logiciels et abonnements pour apprendre, créer et travailler.
          </h1>
          <p className="mt-6 max-w-3xl text-lg leading-8 text-slate-300">
            Consultez les informations essentielles, puis échangez directement avec un conseiller sur WhatsApp pour confirmer le tarif, la disponibilité et le mode d’activation.
          </p>
          ...
          <div className="mx-auto flex max-w-7xl gap-2 overflow-x-auto pb-1">
            {categories.map((category) => (
              <a
                key={category.id}
                href={`#${category.slug}`}
                className="shrink-0 rounded-full border border-slate-200 px-4 py-2 text-sm font-bold text-slate-700 transition hover:border-ocean hover:text-ocean"
              >
                {category.name}
              </a>
            ))}
        ...
        <div className="mx-auto max-w-7xl space-y-20">
              ...
              <div className="rounded-[2rem] bg-gradient-to-br from-ink to-[#0B4268] p-7 text-white shadow-xl sm:p-10">
                <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr] lg:items-center">
                  <div>
                    ...
                    <a
                      href={whatsappUrl(settings.whatsapp, flagship[0].whatsappMessage || `Bonjour GamaDigit, je souhaite des informations sur ${flagship[0].name}.`)}
                      target="_blank"
                      rel="noreferrer"
                      className="mt-7 inline-flex items-center gap-2 rounded-xl bg-mint px-5 py-3 font-black text-white"
                    >
                  ...
                  <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-1">
                    {flagship.map((product) => (
                      <div key={product.id} className="rounded-2xl border border-white/10 bg-white/10 p-6 backdrop-blur">
                        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <section className="rounded-[2rem] bg-white p-8 text-center shadow-sm sm:p-12">
            ...
            <p className="mx-auto mt-4 max-w-2xl leading-7 text-slate-600">
              Envoyez simplement le nom du produit, la durée et le nombre d’appareils ou d’utilisateurs. Notre équipe vérifie la disponibilité.
            </p>
            <a
              href={whatsappUrl(settings.whatsapp, 'Bonjour GamaDigit, je cherche un logiciel qui ne figure pas encore dans le catalogue.')}
              target="_blank"
              rel="noreferrer"
              className="mt-7 inline-flex items-center gap-2 rounded-xl bg-ink px-6 py-4 font-black text-white"
            >
              Demander un autre produit <ArrowRight className="h-5 w-5" />
            </a>
            <div className="mt-6">
              <Link href="/contact" className="text-sm font-black text-ocean">Ou envoyer une demande détaillée</Link>
            </div>
          </section>

          <p className="text-center text-xs leading-6 text-slate-500">
            Prix et disponibilités à confirmer au moment de la commande. Les modalités d’activation, l’éligibilité et les fonctionnalités varient selon l’offre choisie.
          </p>
        </div>
      </main>
    </>

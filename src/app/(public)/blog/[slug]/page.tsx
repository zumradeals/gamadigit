...
export default async function BlogArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <article>
      ...
      <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6">{post.content.map((paragraph, index) => <p key={`${post.id}-${index}`} className="mb-7 text-lg leading-9 text-slate-700">{paragraph}</p>)}</div>
    </article>
  );
}
